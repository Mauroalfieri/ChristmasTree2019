...
function sendData(target) { 
    ...
    document.body.style.backgroundColor = picker.selectedInput.value; 
    var color = hexToRgb(picker.selectedInput.value); 
    var url = "setcolor.html"; 
    url = replaceUrlParam( url , "r", color.r); 
    url = replaceUrlParam( url , "g", color.g); 
    url = replaceUrlParam( url , "b", color.b); 
    url = replaceUrlParam( url , "hex", picker.selectedInput.value); 
    AjaxCall(url); 

    if ( picker.selectedInput.value != "#000000" ) document.getElementById("oldHexCode").value = picker.selectedInput.value;
    picker.editInput(target);
}
...
function AjaxCall(serverFile=null,async=true,timeout='5000') {
    if (serverFile==''|| serverFile==null) { return; }
